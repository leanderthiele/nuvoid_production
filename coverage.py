...
if __name__ == '__main__' :
    ...
    num_batches = int(np.ceil(len(chain_fnames) / batch_size))

    obs_idx = []
    cosmo_idx = []
    oma = []

    for batch_idx in range(num_batches) : # keep looping over batches
        ...
        obs_idx.extend(r[0] for r in results)
        cosmo_idx.extend(r[1] for r in results)
        oma.extend(r[2] for r in results)

        np.savez(f'{filebase}/oneminusalpha_v{version}_{compression_hash}_{model_ident}.npz',
                 param_names=param_names,
                 obs_idx=np.array(obs_idx),
                 cosmo_idx=np.array(cosmo_idx),
                 oneminusalpha=np.array(oma))

        print(f'Batch {batch_idx} / {num_batches} done!')

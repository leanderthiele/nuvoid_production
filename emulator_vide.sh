...
logdir="$wrk_dir/emulator/$hod_hash/logs_$augment"
figdir="$wrk_dir/emulator/$hod_hash/figs"
mkdir -p "$logdir" "$figdir"
vide_cfg="$wrk_dir/emulator/$hod_hash/vide_cfg_$augment.py"

cp $VIDE_CFG_TEMPLATE $vide_cfg
utils::replace $vide_cfg 'inputDataDir'    "$wrk_dir/emulator/$hod_hash"
utils::replace $vide_cfg 'workDir'         "$wrk_dir/emulator/$hod_hash"

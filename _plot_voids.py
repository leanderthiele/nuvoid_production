...
mask = (z_voids>z0-0.5*deltaz_voids) * (z_voids<z0+0.5*deltaz_voids)
...
ax.plot(ra_gal, dec_gal, linestyle='none', marker='o', markersize=0.1)
for ra, dec, R in zip(ra_voids, dec_voids, R_voids) :
    c = plt.Circle((ra, dec), radius=R, fill=False, edgecolor='green')
    ax.add_patch(c)

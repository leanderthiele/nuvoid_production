...
def report_state(running_idx, sim_idx, lightcone_idx, state) :
    subprocess.run(f'{codebase}/mysql_driver end_{TABLE}_plk {VERSION} {running_idx} {sim_idx} {lightcone_idx} {state}',
...
with NBL.TaskManager(cpus_per_task=CPUS_PER_TASK, use_all_cpus=True) as tm :
    ...
    for ii in tm.iterate(list(range(10000))) :
        ...
        if tm.comm.rank == 0 :
            ...
            result = subprocess.run(f'{codebase}/mysql_driver create_{TABLE}_plk {VERSION}',
            ...
            subprocess.run(f'{codebase}/mysql_driver start_{TABLE}_plk {VERSION} {running_idx} {sim_idx} {lightcone_idx}',
                           shell=True, check=True)

...
#   [2] work directory (i.e. something that'll be followed by lightcones)
...
os.makedirs(f'{wrk_dir}/lightcones/{hod_hash}', exist_ok=True)

# write the hod settings to file
with open(f'{wrk_dir}/lightcones/{hod_hash}/hod.info', 'w') as f :
    f.write(f'hash={hod_hash}\n')
    for a in argv_hod :
        f.write(f'{a}\n')

pyglx.get_galaxies(data_dir, times, f'{wrk_dir}/lightcones/{hod_hash}/galaxies',
                   **kwargs)
